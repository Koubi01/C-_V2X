...
public class TsharkParser
{
    ...
    private List<Packet> ParseTsharkJson(string jsonOutput, string pcapFileName)
    {
        ...
        try
        {
            ...
            foreach (var packetElement in root.EnumerateArray())
            {
                ...
                // Parse GeoNetworking position (ITS-G5 packets)
                if (layers.TryGetProperty("gnw", out var gnwLayer))
                {
                    if (gnwLayer.TryGetProperty("geonw.tsb", out var tsbLayer))
                    {
                        if (tsbLayer.TryGetProperty("geonw.src_pos_tree", out var srcPosTree))
                        {
                            var lat = string.Empty;
                            var lon = string.Empty;

                            if (srcPosTree.TryGetProperty("geonw.src_pos.lat", out var latElement))
                            {
                                lat = GetStringValue(latElement);
                            }

                            if (srcPosTree.TryGetProperty("geonw.src_pos.long", out var lonElement))
                            {
                                lon = GetStringValue(lonElement);
                            }

                            if (!string.IsNullOrEmpty(lat) && !string.IsNullOrEmpty(lon))
                            {
                                // Convert from 1/10 micro degree to decimal degrees
                                if (double.TryParse(lat, NumberStyles.Float, CultureInfo.InvariantCulture, out var latVal) &&
                                    double.TryParse(lon, NumberStyles.Float, CultureInfo.InvariantCulture, out var lonVal))
                                {
                                    var latDeg = latVal / 10000000.0;
                                    var lonDeg = lonVal / 10000000.0;
                                    packet.SourceIp = $"{latDeg:F6},{lonDeg:F6}";
                                }
                            }
                        }
                    }
                ...
                // Fallback: Parse traditional IP addresses if present (for IP-encapsulated V2X)
                if (string.IsNullOrEmpty(packet.SourceIp) && layers.TryGetProperty("ip", out var ipLayer))
                {
                    if (ipLayer.TryGetProperty("ip.src", out var sourceIpElement))
                    {
                        packet.SourceIp = GetStringValue(sourceIpElement);
                    }

                    if (ipLayer.TryGetProperty("ip.dst", out var destIpElement))
                    {
                        packet.DestinationIp = GetStringValue(destIpElement);
                    }
                }
                ...
                // Fallback: Parse UDP ports if present (for IP-encapsulated V2X)
                if (packet.Protocol != "BTP" && layers.TryGetProperty("udp", out var udpLayer))
                {
                    if (udpLayer.TryGetProperty("udp.srcport", out var udpSrcPortElement))
                    {
                        if (int.TryParse(GetStringValue(udpSrcPortElement), NumberStyles.Integer, CultureInfo.InvariantCulture, out var port))
                        {
                    ...
                    if (udpLayer.TryGetProperty("udp.dstport", out var udpDstPortElement))
                    {
                        if (int.TryParse(GetStringValue(udpDstPortElement), NumberStyles.Integer, CultureInfo.InvariantCulture, out var port))
                        {
                            packet.DestinationPort = port;
                            packet.Protocol = "UDP";
                        }
                ...
                // Fallback: Parse TCP ports if present
                if (packet.Protocol != "BTP" && packet.Protocol != "UDP" && layers.TryGetProperty("tcp", out var tcpLayer))
                {
                    if (tcpLayer.TryGetProperty("tcp.srcport", out var tcpSrcPortElement))
                    {
                        if (int.TryParse(GetStringValue(tcpSrcPortElement), NumberStyles.Integer, CultureInfo.InvariantCulture, out var port))
                        {
                            packet.SourcePort = port;
                            packet.Protocol = "TCP";
                        }
                    ...
                    if (tcpLayer.TryGetProperty("tcp.dstport", out var tcpDstPortElement))
                    {
                        if (int.TryParse(GetStringValue(tcpDstPortElement), NumberStyles.Integer, CultureInfo.InvariantCulture, out var port))
                        {
                            packet.DestinationPort = port;
                            packet.Protocol = "TCP";
                        }
                ...
                // Determine packet type (simplified V2X detection)
                packet.PacketType = DeterminePacketType(packet, layers);

                // Build normalized payload JSON from nested ITS layers.
                packet.Payload = CreateNormalizedPayload(layers);
    ...
    private static string CreateNormalizedPayload(JsonElement layers)
    {
        ...
        // DENM (Decentralized Environmental Notification Message) fields
        ...
        // SPATEM (Signal Phase and Timing) fields
        ...
        // SREM (Signal Request Message) fields
        ...
        // SSEM (Signal Status Extension Message) fields
    ...
    private string DeterminePacketType(Packet packet, JsonElement layers)
    {
        // Priority 1: Check ITS messageId for direct detection (ITS-G5 packets)
        if (layers.TryGetProperty("its", out var itsLayer))
        {
            if (itsLayer.TryGetProperty("its.ItsPduHeader_element", out var headerElement))
            {
                if (headerElement.TryGetProperty("its.messageId", out var msgIdElement))
                {
                    if (int.TryParse(GetStringValue(msgIdElement), NumberStyles.Integer, CultureInfo.InvariantCulture, out var messageId))
                    {
        ...
        // Priority 2: Check BTP port (ITS-G5 with BTP layer)
        if (packet.Protocol == "BTP")
        {
        ...
        // Priority 3: Check UDP ports (IP-encapsulated V2X)
        if (packet.Protocol == "UDP")
        {
            int srcPort = packet.SourcePort;
            int dstPort = packet.DestinationPort;

            // ETSI ITS standard UDP port assignments
            if (srcPort == 4729 || dstPort == 4729) return "CAM";
            if (srcPort == 2001 || dstPort == 2001) return "DENM";
            if (srcPort == 4731 || dstPort == 4731) return "MAPEM";
            if (srcPort == 4732 || dstPort == 4732) return "SPATEM";
            if (srcPort == 4733 || dstPort == 4733) return "SREM";
            if (srcPort == 4734 || dstPort == 4734) return "SSEM";
        }

        // Priority 4: 802.11p detection based on MAC address OUI
        if (!string.IsNullOrEmpty(packet.SourceMac) && packet.SourceMac.StartsWith("00:50:C2"))
        {
            return "802.11p";
        }
